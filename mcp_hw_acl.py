...
class IP_Connectivity(aetest.Testcase):
    ...
    def section_test(self,uut1,rtr1,rtr2):
        ...
        #ipv4 ping test
        ping_status_ipv4 = rtr1.execute("ping " +target_ip_1)
        output_value_ipv4 = re.search('Success rate is (\d+)',ping_status_ipv4)
        if int(output_value_ipv4.group(1)) >= 80:
            logger.info("Connectivity between rtr1 and rtr2 sucessfull")
        else:
            self.failed("Ping failed between rtr1 and rtr2 ")
            
        ping_status_ipv4 = rtr1.execute("ping " +target_ip_2)
        output_value_ipv4 = re.search('Success rate is (\d+)',ping_status_ipv4)
        if int(output_value_ipv4.group(1)) >= 80:
            logger.info("Connectivity between rtr1 and rtr2 sucessfull")
        else:
        ...
        #ipv6 ping test
        ping_status_ipv6 = rtr1.execute("ping ipv6 " +rtr2IPv6 )
        output_value_ipv6 = re.search('Success rate is (\d+)',ping_status_ipv6)
        if int(output_value_ipv6.group(1)) >= 80:
            logger.info("ipv6 connectivity between rtr1 and rtr2 sucessfull")
        else:
            self.failed("ICMP IPv6 ping test failed between rtr1 and rtr2")
            
        ping_status_ipv6 = rtr1.execute("ping ipv6 " +rtr2lo101IPv6)
        output_value_ipv6 = re.search('Success rate is (\d+)',ping_status_ipv6)
        if int(output_value_ipv6.group(1)) >= 80:
            logger.info("ipv6 connectivity between rtr1 and rtr2 sucessfull")
        else:
...
class Extended_ACL(aetest.Testcase):
    ...
    def section_test(self, uut1, rtr1, rtr2, uutInt2):
        ...
        if int(output_value_ipv4.group(1)) != 100:
            ...
            logger.info("PingResult is {}".format(output_value_ipv4.group(1)))
...
class CommonCleanup(aetest.CommonCleanup):
    ...
    def section_cleanup(self, uut1, rtr1, rtr2, uutInt2, uutInt1, rtr1Int, rtr2Int):
        looger.info("unconfiguring uut1, rtr1, rtr2")
